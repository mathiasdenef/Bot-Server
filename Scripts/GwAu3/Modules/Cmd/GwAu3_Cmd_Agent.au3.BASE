...
Func GwAu3_Agent_ChangeTarget($a_i_AgentID)
	$a_i_AgentID = GwAu3_Agent_ConvertID($a_i_AgentID)
...
Func GwAu3_Agent_MakeAgentArray($a_i_Type = 0)
    If $a_i_Type < 0 Then
...
Func GwAu3_Agent_TargetNearestEnemy($a_f_MaxDistance = 1300)
    ...
    For $i = 1 To $l_i_MaxAgents
        ...
        Local $l_p_Pointer = GwAu3_Agent_GetAgentPtr($i)
        If $l_p_Pointer = 0 Then ContinueLoop

        ; Check if agent is alive
        If Not GwAu3_Agent_IsAlive($i) Then ContinueLoop

        ; Check if agent is an enemy (simplified check)
        Local $l_i_Allegiance = GwAu3_Agent_GetAgentInfo($i, "Allegiance")
        If $l_i_Allegiance <> 3 Then ContinueLoop ; 3 = enemy

        ; Calculate distance
        Local $l_f_Distance = GwAu3_Agent_GetDistance($i, $l_i_MyID)
    ...
    If $l_i_NearestID > 0 Then
        GwAu3_Agent_ChangeTarget($l_i_NearestID)
...
Func GwAu3_Agent_TargetNearestAlly($a_f_MaxDistance = 1300, $a_b_ExcludeSelf = True)
    Local $l_i_NearestID = 0
    Local $l_f_NearestDistance = $a_f_MaxDistance
    Local $l_i_MyID = GwAu3_Agent_GetMyID()
    Local $l_i_MaxAgents = GwAu3_Agent_GetMaxAgents()
    ...
    For $l_i_Index = 1 To $l_i_MaxAgents
        ...
        ; Check if agent exists
        Local $l_p_Pointer = GwAu3_Agent_GetAgentPtr($l_i_Index)
        If $l_p_Pointer = 0 Then ContinueLoop

        ; Check if agent is alive
        If Not GwAu3_Agent_IsAlive($l_i_Index) Then ContinueLoop

        ; Check if agent is an ally (simplified check)
        Local $l_i_Allegiance = GwAu3_Agent_GetAgentInfo($l_i_Index, "Allegiance")
        If $l_i_Allegiance <> 1 Then ContinueLoop ; 1 = ally

        ; Calculate distance
        Local $l_f_Distance = GwAu3_Agent_GetDistance($l_i_Index, $l_i_MyID)
        If $l_f_Distance < $l_f_NearestDistance Then
            $l_f_NearestDistance = $l_f_Distance
...
Func GwAu3_Agent_ClearTarget()
    Return GwAu3_Agent_ChangeTarget(0)
EndFunc

Func GwAu3_Agent_TargetSelf()
    Local $l_i_MyID = GwAu3_Agent_GetMyID()
    Return GwAu3_Agent_ChangeTarget($l_i_MyID)
EndFunc

;~ Description: Run to or follow a player.
Func GwAu3_Agent_GoPlayer($a_v_Agent)
    Return GwAu3_Core_SendPacket(0x8, $GC_I_HEADER_INTERACT_PLAYER, GwAu3_Agent_ConvertID($a_v_Agent))
EndFunc   ;==>GoPlayer

;~ Description: Talk to an NPC
Func GwAu3_Agent_GoNPC($a_v_Agent)
    Return GwAu3_Core_SendPacket(0xC, $GC_I_HEADER_INTERACT_LIVING, GwAu3_Agent_ConvertID($a_v_Agent))
EndFunc   ;==>GoNPC

;~ Description: Run to a signpost.
Func GwAu3_Agent_GoSignpost($a_v_Agent)
    Return GwAu3_Core_SendPacket(0xC, $GC_I_HEADER_GADGET_INTERACT, GwAu3_Agent_ConvertID($a_v_Agent), 0)
EndFunc   ;==>GoSignpost

;~ Description: Attack an agent.
Func GwAu3_Agent_Attack($a_v_Agent, $a_b_CallTarget = False)
    Return GwAu3_Core_SendPacket(0xC, $GC_I_HEADER_ACTION_ATTACK, GwAu3_Agent_ConvertID($a_v_Agent), $a_b_CallTarget)
EndFunc   ;==>Attack

;~ Description: Call target.
Func GwAu3_Agent_CallTarget($a_v_Target)
    Return GwAu3_Core_SendPacket(0xC, $GC_I_HEADER_CALL_TARGET, 0xA, GwAu3_Agent_ConvertID($a_v_Target))
EndFunc   ;==>CallTarget

;~ Description: Cancel current action.
Func GwAu3_Agent_CancelAction()
